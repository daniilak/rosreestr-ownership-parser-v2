...
class IR_EGRN{
    ...
    public function __construct($config){
        ...
        // название куки файла
        $this->cookieFile = 'ir_egrn_'.$this->rosreestrKey.'.cookie';
    }
    ...
    public function login(){
        ...
        do {
            ...
            if ($responseCode >= 500) {
                ...
                // @todo limit retry
                print "Request failed, error {$responseCode}. Retry 110\n";
            }
            $isError = false;
        ...
        $repaintOptions = http_build_query([
            ...
            'cw' => 1903, // window width,
            'ch' => 463, // window height
            'vw' => 753, // parent width
            'vh' => 1, // parent height
            'fr' => '',
        ...
        $locationA = explode('!/',$this->location);
        $this->location = $locationA[0].'!/'.$appConfig['portletUidlURLBase'];
        $this->portletFullURL = $this->baseURL . $this->location;
    ...
    public function getResult($queryId){
        ...
        try {
            $locationA = explode('!/',$this->location);
            $this->location = $locationA[0].'!/'.$this->findSrcByIcon($searchFormData, 'theme://img/download.png');
            $zipSrc = $this->baseURL . $this->location;
            ...
            do {
                ...
                if ($responseCode >= 500 || empty($zipFile)) {
                    // @todo limit retry
                    print "Request failed, error {$responseCode}. Retry 282\n";
                }
    ...
    public function createRequest($cadastralNo, $reg_ion, $keyoptiongroup){
        while(1) {
            try {
                ...
                $cityList = $this->vaadinQuery('windowName=1',
                    "{$this->appLogin['Vaadin-Security-Key']}\x1D{$reg_ion}\x1F{$regionField}\x1Ffilter\x1Fs\x1E0\x1F{$regionField}\x1Fpage\x1Fi"
                );
                $dropDownKey = $this->findKeyByCaption($cityList, $reg_ion);
                $this->vaadinQuery('windowName=1',
                ...
                do {
                    ...
                    try {
                        ...
                        if  ($exception->getMessage() == 'Event itemClick not found!') {
                            $this->counter = 105; 
                        } else {
                            var_dump($exception->getMessage() . "433 строчка");
                            $this->counter += 1;
                        }
                ...
                try {
                    $locationA = explode('!/',$this->location);
                    $this->location = $locationA[0].'!/'.$this->findSrcByContentType($result, 'application/octet-stream');
                    $captchaSrc = $this->baseURL . $this->location;
                } catch (Throwable $exception) {
                    $this->counter = 100;
                }
                $keyoptiongroup = intval($keyoptiongroup);
                $r = $this->vaadinSetKeyText($this->findButton2ByCaption($result, 'optiongroup'), $keyoptiongroup);
                do {
                    curl_setopt($this->ch, CURLOPT_URL, $captchaSrc);
                    curl_setopt($this->ch, CURLOPT_POST, 0);
                    curl_setopt($this->ch, CURLOPT_HTTPHEADER, [
                    ...
                    do {
                        ...
                        if ($responseCode >= 500 || empty($captcha)) {
                            // @todo limit retry
                            print "Request failed, error {$responseCode}. Retry 469\n";
                            usleep(500000);
                        }
                ...
                // print "Финальная пауза $this->rosreestrInterval секунд между запросами в Росреестр\n";
                ...
                try {
                    ...
                    $ree = $this->findTextRegexp($result, 'Превышен интервал между запросами');
                    if ($ree != '') {
                    } else {
                        // var_dump("Другая фигня");
                    }
                    return -102;
                }
            } catch (Throwable $exception) {
    ...
    protected function findTextRegexp(array $data, $text)
    {
        $a =  json_encode($data, JSON_UNESCAPED_UNICODE);
        if (preg_match('/(Превышен интервал между запросами)/', $a , $matches)) {
            return $matches[0];
        } else {
            return "";
        }
    ...
    protected function vaadinQuery($uri, $data){
        ...
        do {
            ...
            if ($responseCode >= 500) {
                // @todo limit retry
                print "Request failed, error {$responseCode}. Retry 677\n";
            }
    ...
    protected function findButton2ByCaption(array $data, $caption)
    {
        foreach ($data as $k => $v) {
            if (is_array($v)) {
                try {
                    return $this->findButton2ByCaption($v, $caption);
                } catch (Throwable $exception) {
                    // it's ok, do nothing
                }
            } elseif ($k === 'type' && $v === $caption) {
                return $data['id'];
            }
        }
        throw new RuntimeException("Button {$caption} not found!");
    ...
    protected function vaadinSetKeyText($id, $key)
    {
        $this->vaadinQuery('windowName=1',
        "{$this->appLogin['Vaadin-Security-Key']}\x1D{$key}\x1C\x1F{$id}\x1Fselected\x1Fc"
        );
    }
